...
class PlayState extends MusicBeatState
{
	...
	**/
	...
	public var modchartTexts:Map<String, funkin.data.scripts.FunkinLua.ModchartText> = new Map<String, funkin.data.scripts.FunkinLua.ModchartText>();
	...
	**/
	...
	var stageData:StageFile = StageData.generateDefault();
	...
	**/
	...
	/**
		Reference to the total song length of the Inst. Used by `timeBar`
	**/
	...
	**/
	...
	override public function create()
	{
		...
		// FOR SOME REASON??? this DESTROYS AUTOCOMPLETION???
		#if !display
		if (funkin.scripting.classes.ScriptedStage.listScriptClasses().contains(curStage))
		{
			stage = funkin.scripting.classes.ScriptedStage.init(curStage);
		}
		#end
	...
	public function startVideo(name:String):Void
	{
		...
		final foundFile = Paths.exists(fileName, BINARY);
		
		if (foundFile)
		{
			inCutscene = true;
			var bg = new extensions.FlxBGSpriteEx(FlxColor.BLACK);
			bg.scrollFactor.set();
			bg.cameras = [camHUD];
			add(bg);
		...
		if (ret != Globals.Function_Stop)
		{
			...
			new FlxTimer().start(countdownDelay, (t:FlxTimer) -> {
				...
				startTimer = new FlxTimer().start((Conductor.crotchet / 1000) / playbackRate, function(tmr:FlxTimer) {
					...
					stage.onCountdownTick(swagCounter);
	...
	function firstEventPush(event:EventNote)
	{
		...
		stage.onEventPushedUnique(event);
	}
	...
	override public function update(elapsed:Float)
	{
		...
		if (startedCountdown && !paused) // modern psych
		{
		...
		if (startingSong)
		{
			...
			else if (!startedCountdown) Conductor.songPosition = -Conductor.crotchet * 5 + Conductor.offset;
		}
		
		if (camZooming)
		{
			FlxG.camera.zoom = FlxMath.lerp(defaultCamZoom + defaultCamZoomAdd, FlxG.camera.zoom, Math.exp(-elapsed * 6.25 * camZoomingDecay * playbackRate));
	...
	public function moveCamera(isDad:Bool)
	{
		...
		var displacement = curCharacter?.returnDisplacePoint() ?? FlxPoint.weak();
	...
	override function stepHit()
	{
		...
		if (Math.abs(FlxG.sound.music.time - (Conductor.songPosition - Conductor.offset)) > maxToleratedOffset
			|| (SONG.needsVoices && vocals.getDesyncDifference(Math.abs(Conductor.songPosition - Conductor.offset)) > maxToleratedOffset))
		{
			resyncVocals();
		}
